...
def markAttendance(name):
    with open('Attendance.csv', 'r+') as f:
        ...
        for line in myDataList:
            ...
            # In the database but not in the attendance list
        if name in classNames and name not in nameList:
            now = datetime.now()
            dtString = now.strftime('%H: %M: %S')
            f.writelines(f'\n{name}, {dtString}')
            print(name, " Successfully identified!")
            ...
            # In the database and in the attendance list
        elif name in classNames and name in nameList:
            text = name + " Successfully identified!"
            translated = translator.translate(text, dest=output_lang)
            print(translated.text)
            converted_audio = gtts.gTTS(translated.text, lang=output_lang)
